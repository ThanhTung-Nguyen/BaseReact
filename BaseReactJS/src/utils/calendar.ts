...
export interface IConverter {
  lunar_month_days: number[]
  solar_1_1: number[]
  getBitInt: (data: number, length: number, shift: number) => number
  solarToInt: (y: number, m: number, d: number) => number
  solarFromInt: (g: number) => ISolar
  lunarToSolar: (lunar: ILunar) => ISolar
  solarToLunar: (lunar: ISolar) => ILunar
}

function Lunar(this: ILunar) {
  this.isLeap = false
  this.lunarDay = 0
  this.lunarMonth = 0
  this.lunarYear = 0
  this.convertToString = function () {
    return `${("0" + this.lunarDay).slice(-2)}/${("0" + this.lunarMonth).slice(
      -2
    )}`
  }
}

function Solar(this: ISolar) {
  this.solarDay = 0
  this.solarMonth = 0
  this.solarYear = 0
}

export function LunarSolarConverter(this: IConverter) {
   ...
   * @return {number}
   */
  this.getBitInt = function (data, length, shift): number {
   ...
   * @return {number}
   */
  this.solarToInt = function (y: number, m: number, d: number): number {
    m = (m + 9) % 12
    y = Math.floor(y) - Math.floor(m / 10)
    return (
      365 * y +
      Math.floor(y / 4) -
      Math.floor(y / 100) +
  ...
  this.solarFromInt = function (g) {
    ...
    var solar: ISolar = new Solar()
    solar.solarYear = Math.floor(y)
    solar.solarMonth = Math.floor(mm)
    solar.solarDay = Math.floor(dd)
    return solar
  }

  this.lunarToSolar = function (lunar: ILunar) {
  ...
  this.solarToLunar = function (solar: ISolar) {
    var lunar: ILunar = new Lunar()
